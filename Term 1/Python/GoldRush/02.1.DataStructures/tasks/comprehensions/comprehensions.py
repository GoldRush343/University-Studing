...
def get_region_ids_with_none_replaces_by_default(
    ...
    :return: region ids
    """
    return [value if (value := record.get('RegionID')) is not None else DEFAULT_REGION_ID for record in records]
...
def get_region_id_if_not_none(
    ...
    :return: region ids
    """
    return [record['RegionID'] for record in records if record.get('RegionID') is not None]
